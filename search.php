<?php
	...
	if(isset($_POST["search"]))
	{
		...
		$sqlSearch="SELECT * FROM `trms`.`tblcustomer` WHERE (CONVERT(`Cust_ID` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Name` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Project` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Division` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Unit` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Phone` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Email` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Address` USING utf8) LIKE '%{$keyword}%' OR CONVERT(`Cust_Is_Active` USING utf8) LIKE '%{$keyword}%')";
		$querySearch = mysqli_query($connection, $sqlSearch);
		$searchCount = mysqli_num_rows($querySearch);
	}
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="col-md-12">
                            <div class="au-breadcrumb-content">
                                ...
                                <form id="frmSearch" class="au-form-icon--sm" action="search.php" method="post">
                                    <input name="search" id="search" class="au-input--w300 au-input--style2" type="text" placeholder="Search for datas &amp; reports...">
			...
			<div class="container-fluid">
				...
				<?php
					if ($searchCount==0)
					{
						...
						<div class="page-wrapper" style="margin: 0 auto;text-align: center;"><h1>No customer found for <?php echo strtolower($keyword); ?></h1></div>
						<div class="page-wrapper"><h1>&nbsp;</h1></div>
						<div class="page-wrapper"><h1>&nbsp;</h1></div>
						<!--end replace-->
						<?php
					...
					{
					?>
					<h2><?php echo $searchCount; ?> CUSTOMER<?php if($searchCount==1){echo "";}else{echo "S";} ?> FOUND FOR <?php echo $keyword; ?></h2><br>
						<div class="table-data__tool">
							<strong style="color: white;padding-left: 25px; padding-top: 5px;">SERVICES REQUESTED BY THIS CUSTOMER</strong>
						</div>
						<div class="table-responsive table-responsive-data2">
							<table class="table table-data2">
								...
								{
									$cust_id = $rowSearch['Cust_ID'];
									$cust_name = $rowSearch['Cust_Name'];
									$cust_project = $rowSearch['Cust_Project'];
									$cust_division = $rowSearch['Cust_Division'];
									$cust_unit = $rowSearch['Cust_Unit'];
									$cust_phone = $rowSearch['Cust_Phone'];
									$cust_email = $rowSearch['Cust_Email'];
									$cust_address = $rowSearch['Cust_Address'];
									$cust_active = $rowSearch['Cust_Is_Active'];
									
									$sqlService = "SELECT * from tblService WHERE Cust_ID = '{$cust_id}'";
									$queryService = mysqli_query($connection, $sqlService);
								...
								?>
									<tr class="tr-shadow">
										...
										<td><?php echo $cust_project ?></td>
										<td><?php echo $cust_division ?></td>
										<td><?php echo $cust_unit ?></td>
										<td><span class="badge-info" style="padding:5px;"><?php echo $cust_phone ?></td>
										<td><span class="block-email"><?php echo $cust_email ?></span></td>
